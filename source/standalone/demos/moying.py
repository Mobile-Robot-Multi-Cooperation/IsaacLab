...
def design_scene() -> tuple[dict, list[list[float]]]:
    ...
    # -- Robot
    kinova_arm_cfg = MOYING_MCR_CFG.replace(prim_path="/World/Origin5/Robot")
    kinova_arm_cfg.init_state.pos = (0.0, 0.0, 0.205)
    ...
    # -- Robot
    sawyer_arm_cfg = MOYING_MOR_CFG.replace(prim_path="/World/Origin6/Robot")
    yaw_angle_rad = 3.14

    w = math.cos(yaw_angle_rad / 2)
    x = 0
    y = 0
    z = math.sin(yaw_angle_rad / 2)
    quaternion = (w, x, y, z)

    sawyer_arm_cfg.init_state.pos = (0.0, 0.0, 0.2)
    sawyer_arm_cfg.init_state.rot = quaternion
...
def run_simulator(sim: sim_utils.SimulationContext, entities: dict[str, Articulation], origins: torch.Tensor):
    ...
    while simulation_app.is_running():
        ...
        for robot in entities.values():
            ...
            joint_pos_target = robot.data.default_joint_pos + torch.randn_like(robot.data.joint_pos) * 0.0
